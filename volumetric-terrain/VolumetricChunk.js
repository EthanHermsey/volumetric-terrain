...
export default class VolumetricChunk {
				...
				for ( let x = - loopRadius; x <= loopRadius; x ++ ) {
					...
					if ( d < radius ) {
						...
						if ( this.isInsideGrid( gridPosition ) ) {
							...
							p = this.map( d, 0, radius * 0.75, 1, 0, true );
	...
	addValueToGrid( x, y, z, val ) {
		...
		return this.grid[ gridOffset ] = this.constrain( this.grid[ gridOffset ] + val, - 0.5, 0.5 );
	...
	addScaleValueToGrid( x, y, z, val ) {
		...
		let gridOffset = this.gridIndex( x, y, z );
		const oldValueScale = this.map( Math.abs( this.grid[ gridOffset ] ), 0, 0.5, 0.001, 3 );
		return this.grid[ gridOffset ] = this.constrain( this.grid[ gridOffset ] + ( val * oldValueScale ), - 0.5, 0.5 );
